...
The dataset contains data of each country recorded during the years between 2000 and 2015.\
...
boxplot(life$Hepatitis.B, main = 'Hepatitis B (HepB) Immunization', ylab =
...
boxplot(life$Total.expenditure, main = 'General Government Health Expenditure', ylab =
...
boxplot(life$GDP, main = 'GDP per Capita (in USD)')
boxplot(life$Population, main = 'Country Population')
```

```{r}
par(mfrow = c(2, 2))
boxplot(life$thinness..1.19.years, main = 'Prevalence of Thinness (10-19 Years)', ylab =
          '%')
boxplot(life$thinness.5.9.years, main = 'Prevalence of Thinness (5-9 Years)', ylab =
          '%')
boxplot(life$Income.composition.of.resources, main = 'HDI')
...
In summary, variables chosen by the methods:

| Model Selection Method | `Status` | `Life.expectancy` | `Adult.Mortality` | `infant.deaths` |
|-----------------------:|:--------:|:-----------------:|:-----------------:|:---------------:|
| Forward Selection      |    x     |                   |         x         |        x        |
| Backward Elimination   |    x     |         x         |         x         |        x        |
| Stepwise Selection     |    x     |                   |         x         |                 |

| Model Selection Method | `Alcohol` | `percentage.expenditure` | `Hepatitis.B` | `Measles` | `BMI` |
|-----------------------:|:---------:|:------------------------:|:-------------:|:---------:|:-----:|
| Forward Selection      |     x     |            x             |       x       |     x     |   x   |
| Backward Elimination   |     x     |            x             |       x       |     x     |   x   |
| Stepwise Selection     |     x     |            x             |       x       |           |   x   |

| Model Selection Method | `Polio` | `Total.expenditure` | `Diphtheria` | `HIV.AIDS` | `GDP` |
|-----------------------:|:-------:|:-------------------:|:------------:|:----------:|:-----:|
| Forward Selection      |    x    |          x          |      x       |     x      |       |
| Backward Elimination   |    x    |          x          |      x       |     x      |       |
| Stepwise Selection     |         |                     |      x       |     x      |       |

| Model Selection Method | `Population` | `thinness..1.19.years` | `thinness.5.9.years` |
|-----------------------:|:------------:|:----------------------:|:--------------------:|
